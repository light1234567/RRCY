...
<div class="mt-4 overflow-hidden bg-white rounded-lg shadow-md">
  <div class="p-4 sm:p-6">
    <table class="min-w-full table-auto">
      ...
      <thead>
        <tr class="border-b">
          <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600 uppercase">Name</th>
          <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600 uppercase">Age</th>
          <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600 uppercase">Case Status</th>
          <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600 uppercase">Child Status</th>
          <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600 uppercase">Date Admitted</th>
        </tr>
      </thead>
      ...
      <tbody class="divide-y divide-gray-100">
        ...
        <tr v-for="(client, index) in sortedClients" :key="index"
            class="hover:bg-gray-50 transition-colors duration-100 ease-in-out cursor-pointer"
            @click="handleRowClick(client)">
          <td class="px-4 py-3 text-sm text-gray-800">{{ client.name }}</td>
          <td class="px-4 py-3 text-sm text-gray-700">{{ client.age }}</td>
          <td class="px-4 py-3 text-sm text-gray-600 font-medium">{{ client.case_status }}</td>
          <td class="px-4 py-3 text-sm" :class="getChildStatusClass(client.child_status)">
            {{ client.child_status }}
          </td>
          <td class="px-4 py-3 text-sm text-gray-500">{{ formatDate(client.date_admitted) }}</td>
        </tr>
