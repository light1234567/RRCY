<template>
  <div>
    ...
    <div class="flex justify-end space-x-4">
      ...
      <button v-if="editMode" @click="cancelEdit" type="button" class="px-4 py-2 bg-red-500 text-white rounded">Cancel</button>
    </div>
    ...
    <div class="p-4">
      <table class="w-full border-collapse border">
        <thead>
          <tr>
            <th colspan="6" class="text-center p-2 border">
              <img src="path-to-your-logo.png" alt="DSWD Logo" class="mx-auto mb-2" style="width: 80px; height: auto;">
          ...
          <tr>
            <th colspan="6" class="text-center p-2 border bg-gray-200">
              <h2 class="text-lg font-bold">TRAINING NEEDS ASSESSMENT</h2>
              <div>
                <label class="block">FOR THE:</label>
                <input type="text" v-model="form.for_the" :readonly="!editMode" class="w-full border p-1 text-center">
              </div>
              <div>
                <label class="block mt-2">Date of Admission:</label>
                <input type="date" v-model="form.date_of_admission" :readonly="!editMode" class="w-full border p-1 text-center">
              </div>
            </th>
          </tr>
        </thead>
...
<tr>
  <td colspan="2" class="p-2 border">
    <label>
      <input type="radio" value="Wala Naka eskwela" v-model="form.selectedEducationLevel" :disabled="!editMode">
      Wala Naka eskwela
    </label>
  </td>
  <td colspan="2" class="p-2 border">
    <label>
      ...
      <input type="text" v-model="form.elementaryGrade" :disabled="!editMode || form.selectedEducationLevel !== 'Elementary Level'" class="w-16 border p-1 text-center" placeholder="____">)
    </label>
  </td>
  <td colspan="2" class="p-2 border">
    <label>
      <input type="radio" value="Junior High School Level" v-model="form.selectedEducationLevel" :disabled="!editMode">
      Junior High School Level (Year
      <input type="text" v-model="form.juniorHighYear" :disabled="!editMode || form.selectedEducationLevel !== 'Junior High School Level'" class="w-16 border p-1 text-center" placeholder="____">)
    </label>
  </td>
</tr>
<tr>
  ...
  <td colspan="2" class="p-2 border">
    <label>
      ...
      Senior High School Level (Year
      <input type="text" v-model="form.seniorHighYear" :disabled="!editMode || form.selectedEducationLevel !== 'Senior High School Level'" class="w-16 border p-1 text-center" placeholder="____">)
    </label>
  </td>
  <td colspan="2" class="p-2 border">
    <label>
...
<tr>
  <td colspan="2" class="p-2 border">
    <label>
      ...
      College Level (Year
      <input type="text" v-model="form.collegeYear" :disabled="!editMode || form.selectedEducationLevel !== 'College Level'" class="w-24 border p-1 text-center" placeholder="__________">)
    </label>
  </td>
</tr>
  ...
  <thead>
    <tr>
      <th colspan="8" class="text-center p-2 border">
        <h2 class="text-lg font-bold">Skills Training Nga Gustong Apilan Diri Sulod sa CENTER</h2>
        <p>Pamili ug lima (5) ka skills o kahanas nga gusto nimo matun-an o 
          prioridad nga maapilan diri sa sulod sa center kung diin ang 1 
          pinakagusto nga unang maapilan ug ang 5 ang pinakaulahi nga 
          gustong maapilan o matun-an.
        </p>
      </th>
    </tr>
    <tr class="bg-gray-200">
      <th class="border p-2 text-center">#</th>
      <th class="border p-2 text-center">Training</th>
  ...
  <tbody>
    ...
    <tr v-for="(item, index) in basicTrainings" :key="'basic-' + index">
      ...
      <div class="mt-4 text-center">
        <p class="text-xs">
          <strong>Name and signature of client:</strong><br>
          <input type="text" v-model="form.name" :readonly="!editMode" class="w-full border">        </p>
      </div>
      <!-- Page Numbers -->
      <div class="mt-4 text-center">
        <p class="text-xs">
          DSWD FIELD OFFICE XI PROTECTIVE SERVICES DIVISION REGIONAL REHABILITATION CENTER FOR YOUTH<br>
          PAGE 2 of 2
        </p>
      </div>
    </div>
