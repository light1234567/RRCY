<template>
    <div class="flex flex-col justify-center items-center min-h-screen">
                  ...
                  <ul>
                      <li v-for="(tab, index) in tabs" :key="index">
                          <a
                              ...
                              :class="{
                                  'bg-blue-100 text-blue-700': slctdTab === tab, // Highlighted tab
                                  'text-gray-700': slctdTab !== tab // Non-highlighted tabs
                              }"
      ...
      <div class="p-4 w-full">
          ...
          <div v-else>
              <div v-if="slctdTab === 'Learner Academic Form'"><LearnerAcademicForm/></div>
              <div v-if="slctdTab === 'Learner Assessment Form'"><LearnerAssessmentForm/></div>
          </div>
      </div>
    </div>
  </template>
  ...
  const slctdTab = ref('Learner Academic Form'); // Default tab value
  ...
  const selectTab = (tab) => {
    slctdTab.value = tab;
    localStorage.setItem('slctdTab', tab); // Save the selected tab to localStorage
    isDropdownOpen.value = false;
  };
  
  // Ensure the correct tab is loaded on page refresh
  onMounted(() => {
    const savedTab = localStorage.getItem('slctdTab');
    if (savedTab && tabs.value.includes(savedTab)) {
      slctdTab.value = savedTab;
    }
  });
  </script>
