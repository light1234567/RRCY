  ...
  <div v-if="showVerifyModal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
      <div class="flex items-center mb-4">
        <h3 class="text-lg font-semibold">User Verified</h3>
      </div>
      <p>The user <strong>{{ selectedUser.full_name }}</strong> has been successfully verified.</p>
      <div class="mt-6 flex justify-end">
        <button @click="closeVerifyModal" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Close</button>
      </div>
    </div>
  </div>

  <!-- Unverified Modal Notification -->
  <div v-if="showUnverifyModal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
      <div class="flex items-center mb-4">
        <h3 class="text-lg font-semibold">User Unverified</h3>
      </div>
      <p>The user <strong>{{ selectedUser.full_name }}</strong> has been marked as unverified.</p>
      <div class="mt-6 flex justify-end">
        <button @click="closeUnverifyModal" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Close</button>
      </div>
    </div>
  </div>
